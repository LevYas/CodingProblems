...
namespace CodingProblemsTests.Graphs
{
    ...
    {
        [Theory]
        [MemberData(nameof(ArrsForBipartitionCheck))]
        public void CorrectlyChecksIsGraphBipartite(int[][] graph, bool expected)
        {
            Assert.Equal(expected, BipartitionsFinder.IsGraphBipartite(graph));
        }

        public static TheoryData<int[][], bool> ArrsForBipartitionCheck
        {
            get => new TheoryData<int[][], bool>
                {
                    { "[1,3],[0,2],[1,3],[0,2]".ToArrays(), true },
                    {
        ...
        {
            Assert.Equal(expected, BipartitionsFinder.PossibleBipartition(n, dislikes));
        }
